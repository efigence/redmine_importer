...
class ImporterController < ApplicationController
  ...
  def result
    ...
    CSV.new(iip.csv_data, {:headers=>true,
      ...
      issue.description = description.present? ? description.gsub(/\r\n?|\\n|<br\s*\/?>/, "\n") : issue.description
      issue.category_id = category != nil ? category.id : issue.category_id
      issue.start_date = row[attrs_map["start_date"]].blank? ? issue.start_date : Date.parse(row[attrs_map["start_date"]])
      issue.due_date = row[attrs_map["due_date"]].blank? ? issue.due_date : Date.parse(row[attrs_map["due_date"]])
